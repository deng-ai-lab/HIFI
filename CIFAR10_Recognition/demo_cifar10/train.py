...
def main():
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        trainset = CIFAR10DVS(args.data_dir, train=True, use_frame=True, frames_num=args.T, split_by='number', normalization=None, transform=transform_train) # type: ignore
        train_data_loader = data.DataLoader(trainset, batch_size=args.b, shuffle=True, num_workers=args.j)

        testset = CIFAR10DVS(args.data_dir, train=False, use_frame=True, frames_num=args.T, split_by='number', normalization=None, transform=transform_test) # type: ignore
    ...
    if args.dataset.startswith('cifar'):
        ...
        net = spiking_resnet_imagenet.__dict__[args.model](neuron=neuron_model, num_classes=num_classes, neuron_dropout=args.drop_rate,
                                                           tau=args.tau, v_threshold=args.vth, surrogate_function=surrogate_function, c_in=3)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for frame, label in train_data_loader:
            ...
            if not args.dataset.startswith('cifar'):
