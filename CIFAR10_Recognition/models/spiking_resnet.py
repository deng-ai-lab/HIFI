...
class PreActBlock(nn.Module):
    ...
    def test(self, x):
        spike_fr = []
        x = self.relu1(self.bn1(x))
        spike_fr.append(x.mean().item())
        out = self.conv1(x)

        out = self.relu2(self.bn2(out))
        spike_fr.append(out.mean().item())
        out = self.conv2(out)
...
class PreActResNet(nn.Module):
    ...
    def test(self, x):
        ...
        # out = self.layer1(out)
        for _block in self.layer1:
            out, fr = _block.test(out)
            spike_fr.extend(fr)
            
        # out = self.layer2(out)
        for _block in self.layer2:
            out, fr = _block.test(out)
            spike_fr.extend(fr)

        # out = self.layer3(out)
        for _block in self.layer3:
            out, fr = _block.test(out)
            spike_fr.extend(fr)

        # out = self.layer4(out)
        for _block in self.layer4:
            out, fr = _block.test(out)
            spike_fr.extend(fr)
    ...
    def calc_flops(self):
        img_size = 32
        flops = []
        flops.append(self.conv1.in_channels * self.conv1.out_channels * self.conv1.kernel_size[0] * self.conv1.kernel_size[1] * img_size * img_size)
        for layer in [self.layer1, self.layer2, self.layer3, self.layer4]:
            for block in layer:
                img_size = img_size // block.stride
                flops.append(block.conv1.in_channels * block.conv1.out_channels * block.conv1.kernel_size[0] * block.conv1.kernel_size[1] * img_size * img_size)
                flops.append(block.conv2.in_channels * block.conv2.out_channels * block.conv2.kernel_size[0] * block.conv2.kernel_size[1] * img_size * img_size)
        flops.append(self.linear.in_features * self.linear.out_features)
        return flops
